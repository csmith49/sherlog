module State = struct
    ...
    let of_atoms goal cache = {
        goal = goal; cache = cache;
    }
    ...
    let rec discharge state = match goal state with
        ...
        | atom :: rest -> if CCList.mem ~eq:Atom.equal atom (cache state)
            ...
            else let state = { state with goal = rest;} in 
                Some (atom, state)
    ...
    let extend gs cs state = {
        goal = gs @ (goal state);
        cache = cs @ (cache state);
    }
...
let of_atoms goal cache =
    let state = State.of_atoms goal cache in
        O (state, None)
...
let substitution proof = proof
    ...
    |> CCList.fold_left Substitution.compose Substitution.empty

let to_atoms proof =
    let sub = substitution proof in
    let atoms = proof
        |> witnesses
        |> CCList.map Witness.atom in
    CCList.map (Atom.apply sub) atoms *)
...
let resolve proof rule = let open CCOpt in
    ...
    let state = state
        |> State.extend (Rule.body rule) [Witness.atom witness]
        |> State.apply sub in
