...
def likelihood_ratio(observation, store):
    if observation.is_empty: return torch.tensor(1.0)
    else:
        lls = torch.stack([store[f].log_prob for f in observation.variables])
        return torch.exp(torch.sum(lls)).detach()
