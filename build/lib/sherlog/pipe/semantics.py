...
class ParallelTransform(Semantics[List[T]]):
    """Parallel execution transformation."""

    def __init__(self, semantics : Semantics[T], width : int):
        """Lift the semantisc to support parallel execution."""

        # unit closre
        def unit(value : Any) -> List[T]:
            return [semantics.pipe.unit(value)] * width

        # bind closure
        def bind(callable : Callable[..., T], arguments : List[List[T]]) -> List[T]:
            # generator loops over all arguments in the cart-prod and yields each possibility
            def gen():
                for args in zip(*arguments):
                    yield semantics.pipe.bind(callable, args)
                
            return list(gen())
            
        super().__init__(Pipe(unit, bind), semantics.namespace)