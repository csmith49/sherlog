...
swap(X, Y; Bern[R]) <- swap_rate(X, Y, R).
...
partition([], Y, [], []).
partition(X :: XS, Y, X :: LS, RS) <- swap(X, Y, 1.0), partition(XS, Y, LS, RS).
partition(X :: XS, Y, LS, X :: RS) <- swap(X, Y, 0.0), partition(XS, Y, LS, RS).
...
class SherlogModel(Model):
    def __init__(self):
        self._problem, _ = sherlog.program.loads(SOURCE)

    def fit(self, data : Iterable[Example], epochs : int = 1, learning_rate : float = 0.1, batch_size : int = 10):
        optimizer = sherlog.inference.Optimizer(self._problem, optimizer="sgd", learning_rate=learning_rate)
        
        for batch in sherlog.inference.minibatch(translate_examples(data), batch_size=batch_size, epochs=epochs):
            with optimizer as o:
                o.maximize(batch.objective(self._problem, explanations=1, samples=100))
    ...
    def accuracy(self, example, **kwargs):
        ...
        for possibility in example.output_permutations():
            ...
            possibility_likelihood = self._problem.likelihood(evidence[0])
